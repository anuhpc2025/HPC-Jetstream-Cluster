...
#SBATCH --nodes=4
#SBATCH --ntasks=256             # total MPI ranks
#SBATCH --ntasks-per-node=64     # (alt test: 32, see below)
...
spack load /buou2hh         # your OpenMPI build
spack load hpl %aocc        # HPL linked against AOCL BLAS

# Safety: make sure we're not picking up some other MPI at runtime
...
export BLIS_ENABLE_OPENMP=0        # DO NOT spawn inner threads
...
# NUMA tip: interleave memory so each socket gets bandwidth
# (harmless even if 1-socket VMs)
